...
void overlayHistos(std::string infile)
{
    ...
    for(int idx=0;idx<list_of_histos->GetSize();idx++)
    {
        ...
        TObjString* otoken=(TObjString*)tokens->At(1);
        TString token = otoken->GetString();
    ...
    for(size_t idx=0;idx<histNames.size();idx++)
    {
        ...
        {
            TH1F *h1=(TH1F*)gDirectory->Get(Form("h_%s_%s",histNames.at(idx).c_str(),enumNames[pidx]));
            ...
            leg.AddEntry(Form("h_%s_%s",histNames.at(idx).c_str(),enumNames[pidx]),enumNames[pidx],"L");
        }
        leg.Draw();
